...
### PF for update, and room placement ###
...
# GenPOMDPs.pf_updater(
#     (action, obs) -> ( # Stratified update with the SMCP3 proposal.
#             SMCP3Update(
#                 forward_iterated_grid_proposal, backward_iterated_grid_proposal,
#                 # is_initial_step, params, new_action, new_obs, grid_args, add_to_room
#                 (false, $(esc(PARAMS)), action, obs[:obs], $(esc(_grid_args)), add_to_room),
#                 # is_initial_step, grid_args, add_to_room
#                 (false, $(esc(_grid_args)), add_to_room)
#             )
#             for add_to_room in [true, false]
#     );

#     pre_update = state -> pf_resize!(state, 2)
# )
