...
# Default arguments for the particle filter
include("default_pf_args.jl")
...
# is a distractor.
macro get_pf(pomdp, PARAMS, _grid_args, _t0_grid_args, _resampling_args, set_init_pose, set_step_pose)
    """
    `set_init_pose(pose)` should return the state choicemap for time 0 corresponding to that pose
    `set_step_pose(pose)` does the same, but for the step distribution
    """
    quote
    ...
    GenPOMDPs.pf(
        ...
        obs -> ( # Initialize via a 3-particle SMCP3 proposal.
            ...
            SMCP3Update(
                ...
                (true, $(esc(PARAMS)), nothing, obs[:obs], $(esc(_t0_grid_args)), $(esc(set_init_pose))), # no action before the initial observation
        ...
        (action, obs) -> ( # Update with the SMCP3 proposal.
            SMCP3Update(
                forward_iterated_grid_proposal, backward_iterated_grid_proposal,
                # is_initial_step, params, new_action, new_obs, grid_args
                (false, $(esc(PARAMS)), action, obs[:obs], $(esc(_grid_args)), $(esc(set_step_pose))),
                # is_initial_step, grid_args
...
GenSMCP3.@kernel function forward_iterated_grid_proposal(tr, is_initial_step, params, new_action, new_obs, grid_args, get_pose_choicemap)
    if is_initial_step
        T = 0
    ...
    update_choicemap = choicemap()
    Gen.set_submap!(update_choicemap, state_addr(T), get_pose_choicemap(x′))
